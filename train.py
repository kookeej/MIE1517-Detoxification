...
#######
    ...
    if 't5' in args.base_model_name:
        ...
        train_layers = [total_layers - i for i in range(1, 4)]  # 마지막 3개
        ...
        if args.hybrid:
            for name, param in model.base_model.named_parameters():
                if any(f"layers.{i}." in name for i in train_layers):
                    if any(key in name for key in ["q_proj", "k_proj", "v_proj", "o_proj", "mlp"]):
                        # print(f"[Hybrid Trainable] {name}")
    ...
    train_dataset = ParadetoxDatasetForTrain(train, tokenizer, args.prompt_type, examples_list=train_examples)
    ...
    for epoch in range(args.epochs):
        ...
        if best_valid_score < valid_score:
            ...
            if isinstance(model, T5ForConditionalGeneration):
                ...
                # base + lora 모두 저장
                model.base_model.save_pretrained(f'./checkpoints/best_{args.output_file_name}_base')
                model.save_pretrained(f'./checkpoints/best_{args.output_file_name}_lora')
            else:
